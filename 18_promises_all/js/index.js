...
async function carta() {
    ...
    try {
        ...
        for (let n = 0; n < 5; n++) {
            arrPromises.push(fetch(`https://deckofcardsapi.com/api/deck/${idDoBaralho}/draw/?count=1`));
            await arrPromises[n];
        }

        Promise.all(arrPromises).then((responses) => {
            printCarts(responses);
        })

        async function printCarts(arr) {

            for (let n = 0; n < arr.length; n++) {

                const dados = await arr[n].json();
                console.log(dados);
